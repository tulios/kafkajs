...
const createFetcher = ({
  ...
  const start = async () => {
    if (isRunning) return
    isRunning = true
    const stopPromise = new Promise(resolve => emitter.once('stop', resolve))

    while (isRunning) {
      try {
        const batches = await Promise.any([fetch(nodeId), stopPromise])
        if (isRunning) {
  ...
  const stop = async () => {
    if (!isRunning) return
    isRunning = false
    const endPromise = new Promise(resolve => emitter.once('end', resolve))
    emitter.emit('stop');
    await endPromise;
  }
