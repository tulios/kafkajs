...
const crc32C = buffer => {
  ...
  while (len > 7) {
    const c0 = (buffer[offset + 0] ^ localCrc) & 0xff
    const c1 = (buffer[offset + 1] ^ (localCrc >>>= 8)) & 0xff
    const c2 = (buffer[offset + 2] ^ (localCrc >>>= 8)) & 0xff
    const c3 = (buffer[offset + 3] ^ (localCrc >>>= 8)) & 0xff
    localCrc = T[T8_7_START + c0] ^ T[T8_6_START + c1] ^ (T[T8_5_START + c2] ^ T[T8_4_START + c3])

    const c4 = buffer[offset + 4] & 0xff
    const c5 = buffer[offset + 5] & 0xff
    const c6 = buffer[offset + 6] & 0xff
    const c7 = buffer[offset + 7] & 0xff
  ...
  switch (len) {
    case 7:
      localCrc = (localCrc >>> 8) ^ T[T8_0_START + ((localCrc ^ buffer[offset++]) & 0xff)]
      break
    case 6:
      localCrc = (localCrc >>> 8) ^ T[T8_0_START + ((localCrc ^ buffer[offset++]) & 0xff)]
      break
...
const T8_0_START = 0 * 256
const T8_1_START = 1 * 256
const T8_2_START = 2 * 256
const T8_3_START = 3 * 256
const T8_4_START = 4 * 256
const T8_5_START = 5 * 256
const T8_6_START = 6 * 256
const T8_7_START = 7 * 256
