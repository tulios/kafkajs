...
module.exports = class EncoderV2 {
  ...
  static sizeOfVarLong(value) {
    let longValue = EncoderV2.encodeZigZag64(value)
    let bytes = 1
  ...
  static sizeOfVarIntBytes(value) {
    ...
    if (size < 0) {
      return EncoderV2.sizeOfVarInt(-1)
    }

    return EncoderV2.sizeOfVarInt(size) + size
  }
  ...
  writeEncoder(value) {
    if (value instanceof EncoderV2 !== true) {
  ...
  writeEncoderArray(value) {
    if (!Array.isArray(value) || value.some(v => !(v instanceof EncoderV2))) {
  ...
  writeVarInt(value) {
    const byteArray = []
    let encodedValue = EncoderV2.encodeZigZag(value)
